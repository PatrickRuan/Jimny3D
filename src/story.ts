...
export const STORY_SECTIONS: StorySection[] = [
  ...
  },
  {
    ...
    points: [
      ...
      '這台「方塊車」後來沒有被丟掉——它變成了 Rax 版玩具車模式的原型。',
    ],
  },
  {
  ...
  {
    title: '7. Rax 版、文字貼紙、貼紙換色',
    points: [
      '把原本用來測試的方塊車正式做成「🦖 Rax Jimny」切換鍵，一鍵在正式版跟玩具版之間切換。',
  ...
  {
    title: '10. 輪圈改色一開始也是誤打誤撞',
    points: [
      '第一版「輪圈改色」其實沒有真的生效——挑錯了材質（"Carro_Roda"字面上是葡萄牙文的輪子，實際卻貼在座椅椅背上），使用者回報「完全看不到變化」後才抓出真正的輪圈材質，結果又發現輪圈的可見部分被切成一堆跟保桿、飾條混在一起的破碎 mesh，沒辦法用材質或 mesh 名稱乾淨切出來。',
      '後來使用者又回報「只有右邊兩個輪圈變色，左邊沒反應」——那一版是先用視覺標色法挑出 6 個看起來覆蓋輪圈的 mesh，結果那組 mesh 剛好只涵蓋了車身右側。最後改用車頂同一套邏輯的延伸版：不挑 mesh，而是在 shader 裡判斷每個三角形是否落在「四個輪圈中心點的橢球範圍」內，範圍內才上色——不管輪圈的幾何被切成幾份、分布在哪些 mesh 裡都能正確覆蓋，鍍鉻銘牌（保留車頭 LOGO 和左右 SUZUKI 字樣、拿掉滿版鍍鉻飾條）也是用同一招。',
    ],
  },
];
